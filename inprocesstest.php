...
$status="approved";
$status2="";
$sql="SELECT * FROM testdrive INNER JOIN signin ON testdrive.uname=signin.uname WHERE testdrive.status='".$status."' AND testdrive.status2='".$status2."'";
...
<body>
  ...
  <h2><center>INPROCESS BOOKINGS</center></h2>
  <hr>
  	...
  	if(mysqli_num_rows($run)==0){
        ...
        while($row=mysqli_fetch_array($run)){
            ...
            echo '<td>' .$row['pname']. '</td>';
             ...
             echo '<form method=post action="viewinprocesstest.php">
